...
public class LogChase : LogState {
    ...
    private void FixedUpdate()
    {
        if (chasing)
        {
            rb2d.transform.position = Vector2.MoveTowards(rb2d.transform.position, playerPosition, logModel.chaseSpeed * Time.fixedDeltaTime);

            if (((Vector2)rb2d.transform.position - playerPosition).SqrMagnitude() < logModel.rangeAttack)
            {
                GetComponent<LogAttack>().enabled = true;
                this.enabled = false;
            }
